...
(function () {
    ...
    itensOriginais.forEach((item) => {
      ...
      if (texto.includes('Trazer Própria Comida')) {
        ...
        texto.includes('Pensão Completa')
         ...
         item.setAttribute('data-piuta-hidden','1');
  ...
  function limparTelaAlimentacao() {
    // Remove elementos inseridos por este script quando sair da etapa de alimentação
    document.querySelectorAll('.piuta-restaurante-card, .piuta-taxa-entrega').forEach((el) => el.remove());

    // Reexibe itens que foram escondidos (marcados por nós)
    document.querySelectorAll('[data-piuta-hidden="1"]').forEach((el) => {
      el.style.display = '';
      el.removeAttribute('data-piuta-hidden');
    });
  }
  ...
  const observer = new MutationObserver(function () {
    ...
    if (isEtapaAlimentacao()) {
      ...
      // Se mudou de etapa, remove o que foi inserido na alimentação
      limparTelaAlimentacao();
    }
  });
